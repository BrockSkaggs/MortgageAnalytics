from dash import Dash, html, dcc, callback, clientside_callback, Input, Output, State, Patch, ClientsideFunction
...
def build_summary_card_col(summary_card: LoanSummaryAIO) -> html.Div:
    return html.Div([summary_card], className='col-lg-3')
...
clientside_callback(
    ...
    Output('summary_cards_div', 'data-drag'),
    Input('summary_cards_div', 'id')
)
...
def update_outcomes_chart(_, start_month: int, start_year: int, duration: int, rate: float, amount: float):
    ...
    patch_div.append(build_summary_card_col(summary_card))
